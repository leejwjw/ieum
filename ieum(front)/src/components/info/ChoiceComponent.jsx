...
const ChoiceComponent = ({ initialInterests = [], onInterestChange }) => {
  ...
  const handleInterestClick = (interest) => {
    setSelectedInterests((prev) => {
      const isSelected = prev.find(
        (item) => item.iconName === interest.iconName
      );
      let updatedInterests = isSelected
        ? prev.filter((item) => item.iconName !== interest.iconName)
        : [...prev, interest];

      // 선택된 관심사가 3개를 초과하면 첫 번째 항목을 제거
      if (updatedInterests.length > 3) updatedInterests.shift();

      // 관심사 변경 사항을 부모 컴포넌트에 전달
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {buttonIcons.map((interest, index) => (
        <button
          key={index}
          type="button"
